from Custom_env import custom_environment_v07
...
env=PettingZooEnv(custom_environment_v07.env(ip_address="127.0.0.1"))
...
optim = torch.optim.Adam(actor_critic.parameters(), lr=0.00003)

net2 = Net((512,512,3),4, hidden_sizes=[128, 128, 128,128], device='cuda').to('cuda')
actor2 = Actor(net2, 4, device='cuda').to('cuda')
critic2 = Critic(net2, device='cuda').to('cuda')
actor_critic2 = ActorCritic(actor2, critic2)
optim2 = torch.optim.Adam(actor_critic.parameters(), lr=0.00003)
...
result = onpolicy_trainer(policy,train_collector,test_collector=None, max_epoch=1000, step_per_epoch=1500, step_per_collect=15, episode_per_test=5, batch_size=16,update_per_step=0.85,repeat_per_collect=1,logger=logger,save_checkpoint_fn=save_checkpoint_fn)
