...
class env(AECEnv):
    ...
    def observation(self, agent_):
        ...
        #same spot
        self.drone.simSetSegmentationObjectID(self.agents[0].spot.name, self.agents[0].spot.id, True)
        ...
        try:
            ...
            if agent_.name == 'DroneFollower1':
                ttt=cv2.rectangle(ttt, (int(boxes[0][0]),int(boxes[0][1])), (int(boxes[0][2]),int(boxes[0][3])), (0, 255, 0),-1)
                ttt=cv2.rectangle(ttt, (int(boxes[1][0]),int(boxes[1][1])), (int(boxes[1][2]),int(boxes[1][3])), (0, 0, 255),-1)
                ttt=cv2.rectangle(ttt, (int(boxes[2][0]),int(boxes[2][1])), (int(boxes[2][2]),int(boxes[2][3])), (255, 0, 0),-1)
    ...
    def _do_action(self,agent,action):
        ...
        self.drone.moveByVelocityAsync(pos.x_val + quad_offset[0],pos.y_val + quad_offset[1],0, 1, vehicle_name=agent.name).join()  
        self.drone.hoverAsync(vehicle_name=agent.name).join()
    ...
    def reward(self, agent):
        ...
        distbetween = np.linalg.norm(quad_pt0[0:2]-quad_pt1[0:2])
        #dist = np.sqrt(dist)
        #dist= -dist
        
        if agent.col or self.done_flag or distbetween >10:
            reward = -100
