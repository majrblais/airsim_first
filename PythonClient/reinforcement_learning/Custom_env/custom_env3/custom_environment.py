...
class env(AECEnv):
    def __init__(self,ip_address="127.0.0.1"):
        ...
        #FIRE SPOTS
        #fire positions
        self.detectors = [[np.array([-45.0,20.0,-50.0])],[np.array([30.0,40.0,-50.0])]]
        num_landmark=2
        self.landmarks = [FireSpot() for i in range(num_landmark)]
        for i, landmark in enumerate(self.landmarks):
            landmark.name = f"detector{i+1}"
            landmark.pos = self.detectors[i]
            
        self.landmarks[0].id=30
        self.landmarks[1].id=40
        #####################################################
        
        
        #####################################################
        #assign fire spot
        self.agents[0].spot=self.landmarks[0]
        self.agents[1].spot=self.landmarks[1]
        #####################################################
    ...
    def observation(self, agent_):
        ...
        ttt = np.zeros((512,512,3), np.uint8)
        ttt=cv2.rectangle(ttt, (int(boxes[0][0])-2,int(boxes[0][1])-2), (int(boxes[0][2])+2,int(boxes[0][3])+2), (0, 0, 255),-1)
        ttt=cv2.rectangle(ttt, (int(boxes[1][0])-2,int(boxes[1][1])-2), (int(boxes[1][2])+2,int(boxes[1][3])+2), (0, 255, 0),-1)
        ttt=cv2.rectangle(ttt, (int(boxes[2][0])+5,int(boxes[2][1])+5), (int(boxes[2][2])-5,int(boxes[2][3])-5), (100, 255, 100),-1)
        ttt=cv2.rectangle(ttt, (int(boxes[3][0])+5,int(boxes[3][1])+5), (int(boxes[3][2])-5,int(boxes[3][3])-5), (100, 100, 255),-1)
        ...
        #print(self.agents[self._index_map[agent_.name]].name)
        #print(self.agents[self._index_map[agent_.name]].col)
        
        #print("oooooooooooooo")
        #print(self.agents[self._index_map[agent_.name]].name)
       # print(self.agents[self._index_map[agent_.name]].col)
        #print(self.drone.simGetCollisionInfo(vehicle_name=agent_.name).has_collided)
    ...
    def masking(self, agent):
        ...
        if pos.x_val>=43:
            self.observation_spaces[agent.name]["action_mask"][0]=0
        if pos.y_val>=43:
            self.observation_spaces[agent.name]["action_mask"][1]=0
        if pos.x_val<= -49:
            self.observation_spaces[agent.name]["action_mask"][2]=0
        if pos.y_val<=-49:
            self.observation_spaces[agent.name]["action_mask"][3]=0
    ...
    def observe(self, agent):
        #print(agent)
        #print(self.agents[self._index_map[agent]].col)
    ...
    def state(self):
        states = tuple(self.observation(self.agents[self._index_map[agent.name]]).astype(np.float32) for agent in self.possible_agents)
    ...
    def _do_action(self,agent,action):
        ...
        if action == 0 and pos.x_val<=44:
            quad_offset = (self.step_length, 0, 0)
            #print("act0")
        elif action == 1 and pos.y_val<=44:
            quad_offset = (0, self.step_length, 0)
            #print("act1")
        elif action == 2 and pos.x_val>= -49:
            quad_offset = (-self.step_length, 0, 0)
            #print("act2")
        elif action == 3 and pos.y_val>=-49:
            quad_offset = (0, -self.step_length, 0)
            #print("act3")
            
        self.drone.moveToPositionAsync(pos.x_val + quad_offset[0],pos.y_val + quad_offset[1],pos.z_val, 1, vehicle_name=agent.name).join()  
        self.drone.moveByVelocityAsync(0, 0, 0, 1, vehicle_name=agent.name).join()
        self.drone.hoverAsync(vehicle_name=agent.name).join()
    ...
    def reward(self, agent):
        ...
        quad_pt0 = np.array(list((agent.pos.x_val,agent.pos.y_val,agent.pos.z_val,)))
        ...
        elif agent.pos.x_val>=44 or agent.pos.x_val<= -49 or agent.pos.y_val >=44 or agent.pos.y_val <=-49:
            reward = -25
            print("OoBFollower1")
            self.terminations = {agent: True for agent in self.possible_agents}
        else:
