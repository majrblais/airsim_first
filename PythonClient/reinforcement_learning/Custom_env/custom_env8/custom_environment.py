...
class env(AECEnv):
    def __init__(self,ip_address="127.0.0.1", val=False):
       ...
       #     ##print(landmark.pos)
        ...
        self.possible_agents=["DroneFollower0"]
    ...
    def _setup_flight(self):
        ...
        for agent in self.agents:
            ...
            if agent.name == "Drone":
                self.drone.moveToPositionAsync(offset, 0, -15, 1, vehicle_name=agent.name).join()
                agent.spot=self.current_landmark
                agent.spot.id=30
            else:
                self.drone.moveToPositionAsync(offset, 0, -16, 1, vehicle_name=agent.name).join()   
    ...
    def reward(self, agent):
        ...
        elif agent.pos.x_val>=42 or agent.pos.x_val<= -51 or agent.pos.y_val >=44 or agent.pos.y_val <=-44:
            reward = 0
            ##print("OoBFollower")
            self.terminations = {agent: True for agent in self.possible_agents}
