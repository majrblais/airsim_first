from Custom_env import custom_environment_v04
...
env=PettingZooEnv(custom_environment_v04.env(ip_address="127.0.0.1"))
...
train_envs = DummyVectorEnv([lambda: PettingZooEnv(custom_environment_v04.env(ip_address="127.0.0.1"))])
test_envs = DummyVectorEnv([lambda: PettingZooEnv(custom_environment_v04.env(ip_address="127.0.0.1",val=True))])

train_collector = Collector(policy,train_envs,VectorReplayBuffer(1000, len(train_envs)),exploration_noise=True)
test_collector = Collector(policy, test_envs, exploration_noise=True)


result = offpolicy_trainer(policy,train_collector,test_collector, max_epoch=100, step_per_epoch=1000, step_per_collect=25, episode_per_test=10, batch_size=8,update_per_step=0.5,logger=logger)
