...
class DQN(nn.Module):
    def __init__(self, ln1, ln2, ln3):
        ...
        #self.lin1z = nn.Linear(ln0,32)
        #self.lin2z = nn.Linear(32,64)
        #self.lin3z = nn.Linear(64,128)  
        
        self.conv1z = nn.Conv2d(3, 16, kernel_size=5, stride=2)
        self.bn1z = nn.BatchNorm2d(16)
        self.conv2z = nn.Conv2d(16, 32, kernel_size=5, stride=2)
        self.bn2z= nn.BatchNorm2d(32)
        self.conv3z = nn.Conv2d(32, 32, kernel_size=5, stride=2)
        self.bn3z = nn.BatchNorm2d(32)
        
        self.lin5z = nn.Linear(928,128)
        ...
        self.lin2e = nn.Linear(128,256)
        self.lin3e = nn.Linear(256,128)
        self.lin4e = nn.Linear(128,64)
        self.lin5e = nn.Linear(64,4)


        
    def forward(self,x_i0=None, x_p1=None, x_p2=None, x_p3=None, i1=None, i2=None):
        print("forward")
        i0=x_i0
        #p0=x_p0
        p1=x_p1
        ...
        if i1 == 1:
            ...
            i0 = i0.to(device)
            i0 = F.relu(self.bn1z(self.conv1z(i0)))
            i0 = F.relu(self.bn2z(self.conv2z(i0)))
            i0 = F.relu(self.bn3z(self.conv3z(i0)))
            i0 = i0.view(i0.size(0), -1)
            
            combinedp = torch.cat((p3,i0),1)
            outp = combinedp.view(combinedp.size(0), -1) 
            outp = self.lin5z(outp)
            

        if i2 == 1:
            #follower, follower, leader position
            print("follower")
        ...
        outp = self.lin2e(outp)
        outp = self.lin3e(outp)
        outp = self.lin4e(outp)
        outp = self.lin5e(outp)
        return outp

        
resize = T.Compose([T.ToPILImage(),T.Resize(64, interpolation=Image.CUBIC),T.ToTensor()])
...
def paramgen(policy_net=policy_net, i1=None, i2=None):
    ls1=["conv1z.weight","conv1z.bias", "bn1z.weight","bn1z.bias","conv2z.weight","conv2z.bias","bn2z.weight","bn2z.bias","conv3z.weight","conv3z.bias", "bn3z.weight","bn3z.bias","conv4z.weight","conv4z.bias","bn4z.weight","bn4z.bias","lin5z.weight", "lin5z.bias"]
    ls2=["lin1a.weight","lin1a.bias", "lin2a.weight", "lin2a.bias","lin3a.weight","lin3a.bias","lin1b.weight","lin1b.bias", "lin2b.weight", "lin2b.bias","lin3b.weight","lin3b.bias","lin2d.weight","lin2d.bias"]
...
#for p in paramgen(policy_net,i1=1):
#    pass

#print("================================================")
#for p in paramgen(policy_net,i2=1):
#    pass
...
def select_action(state_p1=None, state_p2=None, state_p3=None,img=None, i1=None,i2=None):
    global steps_done
    ...
    if True:
        with torch.no_grad():
            if i1==1:
                act=policy_net(x_i0=img,x_p3=state_p3,i1=1)
                act1=act.max(1)[1].view(1, 1)
                return act1
                
            if i2==1:
                act=policy_net(x_p1=state_p1,x_p2=state_p2,x_p3=state_p3,i2=1)
...
def optimize_model(i1=None, i2=None):
    ...
    next_state_values2[non_final_mask_p2]=(target_net(x_p1=non_final_next_states_p1, x_p2=non_final_next_states_p2, x_p3=non_final_next_states_p3,i2=1)).max(1)[0].detach()
...
for i_episode in range(num_episodes):
    ...
    for t in count():
        ...
        #print(state.shape)
        #Leader
        action = select_action(img=state_i1, state_p3=state_p3,i1=1)
        ...
        action = select_action(state_p1=state_p1,state_p2=state_p2,state_p3=state_p3,i2=1)
